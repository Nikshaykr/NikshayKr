...
CELL_W = 8
CELL_H = 15
# bright(sparse) -> dark(dense); leading space clears bg
RAMP = " .`:-=+*cs#%@"
...
CONTRAST = 1.15
BRIGHTNESS = 1.0
GAMMA = 0.8          # >1 brightens mids -> face lands in sparser chars
SHARPEN = True
WHITE_FLOOR = 0.65    # luminance above this is forced to blank (space)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">Nikshaykr@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'Nikshaykr@github:~$ whoami <tspan fill="{INK}">Nikshay Kr</tspan></text>')
parts.append(f'<rect x="{PAD+270}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
